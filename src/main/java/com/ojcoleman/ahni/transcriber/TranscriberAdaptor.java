...
/**
 * A base class for neural network transcribers that may define neuron or
 * synapse model parameters. Some settings are specific to HyperNEAT type
 * encoding schemes.
 */
public abstract class TranscriberAdaptor<T extends Activator> implements Transcriber<T> {

    /**
     * The minimum weight values in the substrate network. If this is not
     * specified then the negated value of HYPERNEAT_CONNECTION_WEIGHT_MAX will
     * be used.
     */
    ...
    /**
     * Property to specify which neuron model to use. Allowable values are
     * implementation dependent.
     */
    public static final String SUBSTRATE_NEURON_MODEL = "ann.transcriber.neuron.model";
    /**
     * Property to specify the available parameters for neurons. Allowable
     * values are implementation dependent. If HyperNEAT is being used then the
     * CPPN may have an output for each parameter.
     */
    public static final String SUBSTRATE_NEURON_MODEL_PARAMS = "ann.transcriber.neuron.model.params";
    /**
     * Optional property to allow specifying that instead of parameter values
     * being defined individually for each neuron, there will be a set of
     * parameter value collections which can be thought of as defining a class
     * of neuron as determined by the specific parameter values (which will
     * determine the behaviour of that class of neuron). Each neuron should then
     * reference one of these collections/classes and have its parameters set
     * accordingly. If this is set to 0 or not specified then each neuron should
     * be assigned parameters values directly (e.g. via an output for each
     * parameter from a CPPN in a HyperNEAT encoding scheme), if this is set to
     * a value greater than 0 then it should specify the number of classes.
     */
    public static final String SUBSTRATE_NEURON_MODEL_PARAMS_CLASSES = "ann.transcriber.neuron.model.params.classes";
    /**
     * Property to specify minimum values for the parameters for each neuron.
     * Allowable values are implementation dependent.
     */
    public static final String SUBSTRATE_NEURON_MODEL_PARAMS_MIN = "ann.transcriber.neuron.model.params.min";
    /**
     * Property to specify maximum values for the parameters for each neuron.
     * Allowable values are implementation dependent.
     */
    public static final String SUBSTRATE_NEURON_MODEL_PARAMS_MAX = "ann.transcriber.neuron.model.params.max";
    /**
     * Property to specify the minimum CPPN output required to produce a
     * non-zero parameter value. Actual parameter values will scale from [0,
     * ann.transcriber.neuron.model.params.max] or
     * [ann.transcriber.neuron.model.params.min, 0].
     */
    ...
    /**
     * Property to specify allowable neuron model types. Typically: the CPPN
     * only needs to specify the type to use if there is more than one type; if
     * there are only two neuron types then a single CPPN output will be used to
     * specify which to use, otherwise a "whichever CPPN output has the highest
     * output value" encoding will be used to specify the type. Allowable values
     * are implementation dependent, however by default it is assumed that a
     * parameter name followed by one or more integer values to specify the
     * allowable types is used (this assumes that the model is set-up with a
     * parameter that can take on integer values to decide what type the model
     * is).
     */
    public static final String SUBSTRATE_NEURON_MODEL_TYPES = "ann.transcriber.neuron.model.types";

    /**
     * Property to specify which synapse model to use. Allowable values are
     * implementation dependent.
     */
    public static final String SUBSTRATE_SYNAPSE_MODEL = "ann.transcriber.synapse.model";
    /**
     * Property to specify the available parameters for synapses. Allowable
     * values are implementation dependent. If HyperNEAT is being used then the
     * CPPN may have an output for each parameter.
     */
    ...
    /**
     * Optional property to allow specifying that instead of parameter values
     * being defined individually for each synapse, there will be a set of
     * parameter value collections which can be thought of as defining a class
     * of synapse as determined by the specific parameter values (which will
     * determine the behaviour of that class of synapse). Each synapse should
     * then reference one of these collections/classes and have its parameters
     * set accordingly. If this is set to 0 or not specified then each synapse
     * should be assigned parameters values directly (e.g. via an output for
     * each parameter from a CPPN in a HyperNEAT encoding scheme), if this is
     * set to a value greater than 0 then it should specify the number of
     * classes.
     */
    public static final String SUBSTRATE_SYNAPSE_MODEL_PARAMS_CLASSES = "ann.transcriber.synapse.model.params.classes";
    /**
     * Property to specify minimum values for the parameters for each synapse.
     * Allowable values are implementation dependent.
     */
    public static final String SUBSTRATE_SYNAPSE_MODEL_PARAMS_MIN = "ann.transcriber.synapse.model.params.min";
    /**
     * Property to specify maximum values for the parameters for each synapse.
     * Allowable values are implementation dependent.
     */
    ...
    /**
     * Property to specify which parameter in the synapse model will be set to 0
     * if the connection should not be enabled or plastic. This is typically
     * applied to a "learning rate" parameter. Usage and allowable values are
     * implementation dependent. The criteria for whether a synapse should be
    ...
    /**
     * Property to specify allowable synapse model types. Typically: the CPPN
     * only needs to specify the type to use if there is more than one type; if
     * there are only two synapse types then a single CPPN output will be used
     * to specify which to use, otherwise a "whichever CPPN output has the
     * highest output value" encoding will be used to specify the type.
     * Allowable values are implementation dependent, however by default it is
     * assumed that a parameter name followed by one or more integer values to
     * specify the allowable types is used (this assumes that the model is
     * set-up with a parameter that can take on integer values to decide what
     * type the model is).
     */
    ...
    /**
     * Subclasses may use this to know whether multiple neuron types are enabled
     * and available.
     */
    protected boolean neuronTypesEnabled;
    /**
     * Subclasses may use this to know whether multiple synapse types are
     * enabled and available.
     */
    protected boolean synapseTypesEnabled;

    /**
     * Subclasses may use this to know whether neuron model parameters are
     * enabled and available.
     */
    protected boolean neuronParamsEnabled = false;
    /**
     * Subclasses may use this to know whether synapse model parameters are
     * enabled and available.
     */
    ...
    /**
     * The value range for each neuron model parameter (neuronModelParamsMax -
     * neuronModelParamsMin).
     */
    protected double[] neuronModelParamsRange;
    /**
     * the minimum (possibly CPPN) output required to produce a non-zero
     * parameter value. Actual parameter values are typically scaled from [0,
     * ann.transcriber.neuron.model.params.max] or
     * [ann.transcriber.neuron.model.params.min, 0]. Not always implemented.
     */
    ...
    /**
     * The value range for each synapse model parameter
     * (synapseModelParamsMax[i] - synapseModelParamsMin[i]).
     */
    ...
    /**
     * The name of the parameter in the neuron model that specifies the type of
     * a neuron.
     *
    ...
    /**
     * The name of the parameter in the synapse model that specifies the type of
     * a synapse.
     *
    ...
    /**
     * The allowable values for the {@link #neuronModelTypeParam} model
     * parameter that specify the type of a neuron.
     *
     * @see TranscriberAdaptor#SUBSTRATE_NEURON_MODEL_TYPES
     * @see TranscriberAdaptor#SUBSTRATE_NEURON_MODEL_PARAMS
     */
    ...
    /**
     * The allowable values for the {@link #synapseModelTypeParam} model
     * parameter that specify the type of a synapse.
     *
     * @see TranscriberAdaptor#SUBSTRATE_SYNAPSE_MODEL_TYPES
     * @see TranscriberAdaptor#SUBSTRATE_SYNAPSE_MODEL_PARAMS
     */
    ...
    /**
     * Sub-classes can override this method to convert the genotype to a
     * phenotype, using an existing substrate object to allow performance gains
     * by avoiding destroying and creating large objects or arrays, and
     * optionally providing additional options.
     *
    ...
    /**
     * May be overridden to specify how many input neurons the Chromosome should
     * encode. This default implementation returns the value specified by the
     * property {@link com.anji.neat.NeatConfiguration#STIMULUS_SIZE_KEY} if
     * given, otherwise
     * {@link com.anji.neat.NeatConfiguration#DEFAULT_STIMULUS_SIZE}.
     */
    ...
    /**
     * May be overridden to specify how many output neurons the Chromosome
     * should encode. This default implementation returns the value specified by
     * the property {@link com.anji.neat.NeatConfiguration#RESPONSE_SIZE_KEY} if
     * given, otherwise
     * {@link com.anji.neat.NeatConfiguration#DEFAULT_RESPONSE_SIZE}.
     */
    ...
    /**
     * Gets the neuron parameter values for each parameter in each parameter
     * class for the specified chromosome, in the format [class][param] (param
     * order same as {@link #neuronParamNames}).
     *
     * @see #SUBSTRATE_NEURON_MODEL_PARAMS_CLASSES
     */
    ...
    /**
     * Gets the synapse parameter values for each parameter in each parameter
     * class the specified chromosome, in the format [class][param] (param order
     * same as {@link #synapseParamNames}).
     *
     * @see #SUBSTRATE_SYNAPSE_MODEL_PARAMS_CLASSES
     */
    ...
    /**
     * Returns a list of {@link Allele}s, or more importantly
     * {@link org.jgapcustomised.Gene}s referenced by the Alleles, that this
     * transcriber knows what to do with. This default implementation adds
     * {@link ParamAllele}s for each neuron and synapse model parameter class
     * (see {@link #SUBSTRATE_NEURON_MODEL_PARAMS_CLASSES} and
     * {@link #SUBSTRATE_SYNAPSE_MODEL_PARAMS_CLASSES}). It also adds the
     * ParamMutationOperator to the list of mutation operators for the given
     * configuration if it hasn't already been added. Sub-classes may override
     * this method to add alternate or additional Alleles/Genes.
     */
